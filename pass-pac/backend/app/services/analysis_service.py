...
def analyze_card(card: DetectedCard) -> AnalysisRuleResult:
    ...
    if context["magic_card_indicator"]:
        return AnalysisRuleResult(
            rule_id="hf_magic_card_indicator",
            title="UID-modifiable card indicator observed",
            description=(
                "Read-only Proxmark metadata reported characteristics associated "
                "with a UID-modifiable test card. This can be significant when an "
                "access-control system relies on a static UID as its trust decision."
            ),
            risk_level="high",
            recommendation=(
                "Confirm the credential is authorized test media and validate that "
                "the scoped reader uses cryptographic credential verification rather "
                "than UID-only authorization."
            ),
        )

    if context["nonce_type"] in {"static", "weak_prng"}:
        return AnalysisRuleResult(
            rule_id="hf_weak_nonce_indicator",
            title="Weak MIFARE nonce behavior observed",
            description=(
                "The family-specific metadata inspection reported static or weak "
                "PRNG nonce behavior. No key recovery or authentication attack was "
                "performed by PASS-PAC."
            ),
            risk_level="high",
            recommendation=(
                "Review whether this credential family is approved, avoid UID-only "
                "trust, and plan migration to credentials with modern cryptographic "
                "authentication where practical."
            ),
        )

    if context["non_unique_uid"]:
        return AnalysisRuleResult(
            rule_id="hf_non_unique_uid",
            title="Non-unique HF identifier observed",
            description=(
                "The card reported a fixed non-unique identifier. This identifier "
                "is not suitable as the sole basis for an access-control decision."
            ),
            risk_level="medium",
            recommendation=(
                "Confirm the scoped reader validates application-level or "
                "cryptographic credential data and does not authorize access using "
                "only the reported UID."
            ),
        )
...
def build_evidence(
    ...
    return {
        ...
        "dataset_correlation": normalized.get("dataset_correlation", {}),
...
def build_analysis_context(card: DetectedCard) -> dict[str, Any]:
    ...
    analysis_fields = _as_dict(normalized.get("analysis_fields"))
    dataset_correlation = _as_dict(normalized.get("dataset_correlation"))
    ...
    bit_length = _as_int(
        raw_output.get("bit_length")
        or analysis_fields.get("bit_length")
    ...
    nonce_type = _lower_text(inspection_fields.get("nonce_type"))
    magic_card_indicator = bool(inspection_fields.get("magic_card_indicator"))
    non_unique_uid = bool(inspection_fields.get("non_unique_uid"))
    ...
    is_basic_lf_identifier = (
        any(keyword in card_type for keyword in ["em4100", "em410x", "em 410x", "tk4100"])
        or any(keyword in protocol for keyword in ["em4100", "em410x", "tk4100"])
        or any(keyword in key_type for keyword in ["em4100", "em410x", "tk4100"])
    ...
    return {
        ...
        "magic_card_indicator": magic_card_indicator,
        "non_unique_uid": non_unique_uid,
        "dataset_best_score": _as_int(dataset_correlation.get("best_score")),
        "dataset_confidence": _lower_text(dataset_correlation.get("confidence")),
